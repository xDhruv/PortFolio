...
-- .The app allows the user to predict the winner of the match before the toss happens for the match on which the user is predicting
-- here we see in query,the user who have good at prediction in each match those get 100% won result .Here we need to check bid_status 
-- of every match ther user is bidding on it and we see in query that bidder who won the more match is having top in the chart.  
-- so we can record the percentage of win of each bidder with the above query

--  2. Display the number of matches conducted at each stadium with the stadium name, the city from the database.

SELECT ipl_stadium.stadium_id, stadium_name,city , COUNT(ipl_stadium.stadium_id) AS total_match
FROM ipl_stadium
...
ORDER BY total_match;

-- A major factor behind the fan-following of IPL teams is the stadiums or the home grounds on which the matches are played
-- Every ipl team has to play a game in own home ground stadium and also away .
-- With each stadium bringing in a different experience, it is these venues which give a varying taste to plentiful IPL matches.
-- Here for above query we can see ,In wankhade stadium matches has played most of the time i.e. 18,Followed by Is bindra stadium ,Mohali then Holkar stadium,
-- Indore like that.so we can record the no. of matches conducted by each stadium with the above query
...
-- The insight of above query shows that,it is known fact that the team which won maximum number of toss in ipl matches also win the most number 
-- of matches as well. so,let us countercheck with query,if the team who won the toss team also check with the pitch condition, if its suitable for 
-- batting then the team will definstly goes for batting or the stadium must be favourable for chasing,So  depends on that the team will decides.
...
-- Before the bidder selecting a team they first check the player statistic,then about the playing condition,checking maximum number of all rounder
-- also the team stats,along with the pitch condition,also most of the bidder also create multiple team.
-- In IPL_PIE_IN_THE_SKY also they checking the captain and vice-captain of your team give you the highest point based on that the bidder going to select the team.
...
--  The Indian Premier League is a world best Twenty20 (T20) competition in men's cricket.Also every team has to play 
-- with each other and also we know that most of the team who won the toss he will go for ball first as the most of the pitch or stadium is
-- favourable for chasing or depends on the stadium .So we need to check win details of every match that team paly
-- based on that we have  to check that team who are won the match 
...
-- In ipl every team has to play a match with each other in league stage.For every team has to play 2 match with other team in ipl .
-- If team will play 14 match out of this, for qualifying to next round every team has to won 8 match .Out of 8 teams for booking a place in semi-final round
-- only 4 team will qualify for that.So we can see in above query every team has played a equal no of matches out of this only few team are there
-- who won there most no. of matches .
...
-- ipl is a series of many emerging players who show their skills to attracts selectors of their respective national teams
-- So every team has own important bowler to bowl in any situation of game ,so here we have to find the bowler who only played for mumbai indians
-- so we can find the bowler of mumbai indians by above query

-- 8. How many all-rounders are there in each team, Display the teams with more than 4 the all-rounder in descending order.

SELECT team_name, player_role, COUNT(player_role) AS total_all_rounder
FROM ipl_team
INNER JOIN ipl_team_players
...
-- All rounder for every team has very crucial cause All-rounders get to perform with both bat and bowl for their team and also they win matches for the team
-- If an all-rounder can take one or two wickets while scoring 30-40 odd runs with the bat ,they will definetly win match for the team.
-- in ipl every teams want be play with 3,4 all rounder who take wickets and score in crucial stage .
-- so we can record  the all rounder for every team in descending order with above query.
...
SELECT bid_dt.bid_status,year(bid_dt.bid_date) bidding_year,sum(bdr_pt.total_points) total_bidding_points FROM ipl_bidder_points bdr_pt 
join ipl_bidding_details bid_dt ON bdr_pt.bidder_id=bid_dt.bidder_id
join ipl_match_schedule mtc_schd on bid_dt.schedule_id=mtc_schd.schedule_id where  mtc_schd.stadium_id=7 and bid_team=1
GROUP BY bid_dt.bid_status
order by total_bidding_points DESC;

-- Most of the bidder has selected team before the toss and based on that they seletcted the player ,Also they checked  about the playing condition
-- along with the pitch condition or stadium ALso they checked about team has good win percentage  in which stadium based on that they select the player
-- and team.Also we know CSK has good win percentage in overall ipl matches.
...
SELECT p.player_name,p.performance_dtls,substring(p.performance_dtls,15,16) as top_no_of_wickets,tp.player_role,t.team_name from ipl_player p
join ipl_team_players tp on tp.player_id=p.player_id 
join ipl_team t on t.team_id=tp.team_id
where player_role in ('Bowler','All-rounder') 
group by p.player_name; 

WITH CTE AS 
(SELECT p.player_name,p.performance_dtls,substring(p.performance_dtls,15,16) as top_no_of_wickets,tp.player_role ,t.team_name
from ipl_player p,ipl_team_players tp,ipl_team t
WHERE player_role in ('Bowler','All-rounder') 
group by p.player_name) 
SELECT player_name,team_name,player_role FROM CTE;

# here we unable to do by subquery
-- Ipl is a series of many emerging players who show their skills to attracts selectors of their respective national teams 
-- So every team has own important bowler to bowl in any situation of game.All rounder for every team has very crucial cause All rounders get 
-- to perform with both bat and bowl If an all-rounder can take 4 or 5 wickets in match it completely change the situation of the game

-- 11.	show the percentage of toss wins of each bidder and display the results in descending order based on the percentage

select bdr_dt.bidder_id 'Bidder ID', bdr_dt.bidder_name 'Bidder Name', 
(select count(*) from ipl_match mtc JOIN ipl_match_schedule ms ON mtc.match_id = ms.match_id
where mtc.toss_winner=bdr_dt.bidder_id  and (TOSS_WINNER=BIDDER_id))/ 
(select COUNT(no_of_bids) from ipl_bidder_points bdr_pt 
where bdr_pt.bidder_id = bdr_dt.bidder_id)*100 as 'Percentage of toss Wins (%)'
from ipl_bidder_details bdr_dt order by 'Percentage of toss Wins (%)' desc;

-- The insight of above query shows that,In IPL we know toss is very crucial for both the team,based on that condition they go for 
-- either bating or for chasing And most of the bidder go to bid on toss either he won or loss,
-- so here we need to find the bidder who won the toss for each game played in ipl
...
SELECT tournmt_id,tournmt_name,from_date,to_date,
...
-- The IPL takes place in March, April and May of every year and lasts for approximately seven weeks.There will be total 60 matches played,
-- including 4 playoff, if there will be 8 teams are present in one season,so it takes mostly 51 days to complete one ipl season.
-- so based on that we have to find the maximum and minimum duration of every ipl season played.


-- 13.	Write a query to display to calculate the total points month-wise for the 2017 bid year. sort the results based on 
-- total points in descending order and month-wise in ascending order.
-- Note: Display the following columns:
-- 1.	Bidder ID, 2. Bidder Name, 3. bid date as Year, 4. bid date as Month, 5. Total points
-- Only use joins for the above query queries.


SELECT  distinct bdr_dt.bidder_id,bdr_dt.bidder_name,MONTH(bid_dt.bid_date) AS month_wise,YEAR(bid_dt.bid_date) AS year_wise,bdr_pt.total_points
FROM ipl_bidder_details bdr_dt 
JOIN ipl_bidder_points bdr_pt   ON bdr_dt.bidder_id=bdr_pt.bidder_id
JOIN ipl_bidding_details bid_dt ON bid_dt.bidder_id=bdr_pt.bidder_id 
WHERE YEAR(bid_dt.bid_date) =2017 
ORDER BY bdr_pt.total_points desc,month(bid_dt.bid_date);

-- The app allows the user to predict the winner of the match before the toss happens for the match on which the user is predicting.
-- For every win, users get points. There are no negative points, meaning if the user loses the bid, he or she does not lose his/her points. 
-- The Point system is very dynamic. If the difference in the points between two teams playing, is <= 6, but > 0, then the person who predicts: 
...
WITH CTE AS 
		(select distinct bdr_dt.bidder_id,bdr_dt.bidder_name,month(bid_dt.bid_date) as month_wise,bdr_pt.total_points,
         YEAR(bid_dt.bid_date) AS year_wise
         FROM ipl_bidder_details bdr_dt,ipl_bidding_details bid_dt,ipl_bidder_points bdr_pt
         WHERE bdr_dt.bidder_id=bid_dt.bidder_id AND  bid_dt.bidder_id=bdr_pt.bidder_id AND
         YEAR(bid_dt.bid_date)=2017
         GROUP BY bdr_dt.bidder_id,bdr_dt.bidder_name 
         )
SELECT bidder_id,bidder_name,total_points,ROW_NUMBER()
OVER(ORDER BY total_points DESC,month_wise ASC) FROM CTE; 
...
SELECT  bdr_dt.BIDDER_ID,bdr_dt.BIDDER_NAME AS HIGHEST_3_BIDDERS,
bdr_pt.total_Points AS Total_points
FROM IPL_BIDDER_Details bdr_dt
JOIN IPL_BIDDER_POINTS bdr_pt WHERE 
bdr_dt.BIDDER_ID = bdr_pt.BIDDER_ID
ORDER BY Total_points DESC limit 3;

SELECT  bdr_dt.BIDDER_ID,bdr_dt.BIDDER_NAME AS LOWEST_3_BIDDERS,
bdr_pt.total_Points AS Total_points
FROM IPL_BIDDER_Details bdr_dt
JOIN IPL_BIDDER_POINTS bdr_pt WHERE 
bdr_dt.BIDDER_ID = bdr_pt.BIDDER_ID
...
-- The app allows the user to predict the winner of the match before the toss happens for the match on which the user is predicting 
-- For every win, users get points. There are no negative points, meaning if the user loses the bid, he or she does not lose his/her points 
-- The bidder who got more points he will be on the top of the chart in pie sky app and the bidder who got less points on match the bidder 
-- will be on bottom of the chart in pie sky app.


-- 16.	Create two tables called Student_details and Student_details_backup.Create a trigger in such a way that 
...
INSERT INTO student_details VALUE(1,'jack','jack123@gmail.com','8565854642');
INSERT INTO student_details VALUE(2,'matt hardy','hardy456@gmail.com','8545254642');
...
DELIMITER $$
CREATE TRIGGER student_new_details
...
# we are not sure for this query ,hereA trigger is a special type of stored procedure that automatically runs when an event occurs in the 
-- database server. DML triggers run when a user tries to modify data through a data manipulation language (DML) event. 
